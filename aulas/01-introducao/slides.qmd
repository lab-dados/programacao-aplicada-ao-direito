...
::: {.agenda}
::: {.bloco}
[00:00]{.tempo} Abertura e combinados
:::
::: {.bloco}
[00:08]{.tempo} Por que programar: os três argumentos e o que isso muda no Direito
:::
::: {.bloco}
[00:40]{.tempo} Mão na massa no navegador, até a transcrição
:::
::: {.bloco}
[01:15]{.tempo} Intervalo
:::
::: {.bloco}
[01:25]{.tempo} Fim do notebook
:::
::: {.bloco}
[01:40]{.tempo} Os conceitos, agora com nome
:::
::: {.bloco}
[01:50]{.tempo} Criando o seu projeto com uv, do zero
:::
::: {.bloco}
...
::: {.grade .grade-3}
::: {.cartao}
[**Transparência**]{.titulo}
A parte contrária questiona o número. Você reexecuta a análise na frente de quem
perguntou e mostra de onde ele veio.
:::
...
Ferramenta aberta é construída e corrigida por muita gente. Você parte do que a
comunidade já fez, em vez de começar do zero.
:::
:::
...
::: {.cartao}
[**Feito aqui do lado**]{.titulo}
`juscraper` (dados de tribunais), `juscraper-app` (busca de jurisprudência no
navegador), `labdados` (OCR e transcrição) e o QualiLab (análise qualitativa,
também no navegador) são abertos. Dá para ler o código, abrir um *issue*, propor
uma correção. Vários deles nasceram de uma dúvida de pesquisa como as suas.
:::
...
::: {.atalho}
**Hoje fazemos as duas, nessa ordem.** A prática de daqui a pouco é na estrada 1,
para que ninguém fique parado. Depois do intervalo, levamos o mesmo notebook para
a estrada 2.
:::
...
::: {.cartao}
[**Começar pelo navegador**]{.titulo}
A prática de hoje é no Colab, com um clique, para que a aula não dependa de
nenhuma instalação ter dado certo.
:::
:::

::: {.aviso-pratica}
**O convite fica aberto.** Se a instalação depois do intervalo não sair hoje,
você continua pelo Colab e a gente resolve na monitoria. Até o Encontro 6, quando
entrarmos em agentes de IA, o ambiente local passa de preferência a requisito.
:::

## O que a prática de hoje termina fazendo

Você grava 30 segundos lendo um trecho de uma decisão em voz alta, e cinco linhas
de Python devolvem o texto transcrito.
...
::: {.cartao}
[**Sem chave e sem servidor**]{.titulo}
O modelo roda na máquina em que o notebook está aberto. Não é preciso conta em
serviço nenhum.
:::
...
## Transcrever serve para quê

::: {.grade .grade-2}
::: {.cartao}
[**Sessões de julgamento**]{.titulo}
As plenárias do STF são horas de vídeo por semana. Transcritas, viram texto
pesquisável: dá para procurar um termo, contar quantas vezes um argumento
aparece, acompanhar um tema ao longo de anos.
:::

::: {.cartao}
[**Entrevistas de pesquisa**]{.titulo}
Uma pesquisa qualitativa com 40 entrevistas de uma hora custa semanas de
transcrição manual, ou alguns minutos de processamento.
:::

::: {.cartao}
[**Audiências e depoimentos**]{.titulo}
A gravação vira texto para conferência, citação e análise, em vez de ficar
parada em um arquivo de áudio que ninguém reabre.
:::

::: {.cartao}
[**Sustentações orais**]{.titulo}
O mesmo, para acompanhar teses e argumentos de forma sistemática, e não caso a
caso.
:::
:::
...
::: {.aviso-pratica}
Abra o notebook do Encontro 1 **no Colab**, pelo link da página do curso. Um
clique, sem instalar nada.

No Colab, use `Arquivo` e depois `Salvar uma cópia no Drive`, senão o que você
...
::: {.fonte}
Os slides continuam aqui, e voltamos a eles depois do intervalo. Agora é teclado.
:::

# Os conceitos, agora com nome {.secao background-color="#023A78"}
...
# Levando para o seu computador {.secao background-color="#023A78"}
...
::: {.passos}
::: {.passo}
`uv init programacao-direito` cria a pasta do projeto, já com um repositório Git
dentro.
:::
::: {.passo}
`cd programacao-direito` entra na pasta.
:::
::: {.passo}
`uv add jupyter "labdados[transcricao]"` instala o que a aula usa e anota isso no
`pyproject.toml`.
:::
::: {.passo}
No Colab, `Arquivo` e depois `Fazer download` do `.ipynb`. Salve o arquivo dentro
da pasta do projeto.
:::
:::

::: {.aviso-pratica}
Três comandos e um download. Repare que **nada disso é específico deste curso**:
é assim que se começa qualquer projeto em Python.
:::
...
::: {.passos}
::: {.passo}
`File` e depois `Open Folder...`, escolhendo a pasta do projeto.
:::
::: {.passo}
Instale as extensões **Python** e **Jupyter**, ambas da Microsoft.
:::
::: {.passo}
Abra o notebook que você baixou do Colab.
:::
::: {.passo}
Clique em `Select Kernel` e escolha o `.venv` do projeto.
:::
::: {.passo}
Refaça a Parte C, agora na sua máquina.
:::
:::

::: {.centro}
[O mesmo notebook, o mesmo código, o mesmo resultado. O que mudou é de quem é a
máquina.]{.destaque}
:::
...
::: {.passos}
::: {.passo}
No GitHub, crie um repositório **vazio**, sem README e sem licença.
:::
::: {.passo}
`git branch -M main` renomeia a branch que o `uv` criou, para bater com o nome
que o GitHub usa.
:::
::: {.passo}
`git add .` e `git commit -m "aula 1"` registram o estado atual do projeto.
:::
::: {.passo}
`git remote add origin` com o endereço do seu repositório, e `git push -u origin
main`.
:::
:::

::: {.aviso-pratica}
A partir daqui o repositório é seu. É o começo de um portfólio: ao fim do curso,
ele mostra o que você fez, e não só o que você assistiu.
:::
...
::: {.cartao}
[**O ambiente**]{.titulo}
O notebook rodou no Colab para todo mundo. Quem conseguiu instalar já tem o
repositório próprio rodando também.
:::
...
::: {.cartao}
[**O hábito**]{.titulo}
Ao fim de cada aula: `git add`, `git commit` e `git push`. O projeto cresce junto
com o curso.
:::
:::
...
:::
::: {.passo}
Se o seu projeto ainda não subiu para o GitHub, termine isso em casa. No Encontro
2 faremos *fork* e *clone*.
:::
::: {.passo}
